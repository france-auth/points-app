    ...
    return (
        <Layout>
            <div className='p-3 w-full flex flex-col justify-center items-center min-h-96'>
                ...
                {/** INPUT Form */}
                <form className='flex xs:w-screen-minus-3rem w-full relative'>
                    <input type="text" 
                        placeholder={`Chat with your Clone ${x}/${y}`}
                        name="message" id=""
                        className='w-full mb-0 mt-3 placeholder-[#367378] xs:placeholder:*:text-xs xs:px-2 text-sm xs:font-normal font-medium px-5 py-3 rounded-lg border focus:ring focus:ring-[#367378]'
                    />

                    <button type="submit"
                    className='font-bold xs:mr-1 absolute right-2 mr-3 mt-[6px] top-1/2 transform -translate-y-1/2 cursor-pointer'>SEND</button>
                </form>
            </div>
        </Layout>
    )
